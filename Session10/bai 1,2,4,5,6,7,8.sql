...
VALUES ('linh', 'Le Nhat Linh', 'Nu', 'nlinh@gmail.com', '123', '2000-04-19', 'Hà Nội');
SELECT * FROM view_users_firstname;

-- XOA VIEW
DELETE FROM users WHERE username = 'linh';
SELECT * FROM view_users_firstname;


-- BAI 2: Tạo một view tên view_user_post hiển thị danh sách các User với các cột: user_id(mã người dùng) và total_user_post (Tổng số bài viết mà từng người dùng đã đăng)
CREATE VIEW view_user_post AS
    SELECT 
        user_id, COUNT(post_id) AS total_user_post
    FROM	
        post
    GROUP BY user_id;
...
SELECT 
    u.full_name, COUNT(p.post_id) AS total_user_post
FROM
    users u
        LEFT JOIN
...
-- Xóa chỉ mục trên bảng posts
DROP INDEX idx_created_at_user_id ON posts;

-- Xóa chỉ mục trên bảng users
...
LIMIT 10;
EXPLAIN ANALYZE
SELECT u.username, u.hometown, p.post_id, p.content
FROM users u
JOIN posts p ON u.user_id = p.user_id
WHERE u.hometown = 'Hà Nội'
ORDER BY u.username DESC
